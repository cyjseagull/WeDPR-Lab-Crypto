...
mod tests {
    ...
    fn test_aes256_short_message() {
        ...
        let ciphertext_empty = aes256.encrypt(&msg.to_vec(), &key_empty, &key_iv).unwrap();
        println!("decrypted_msg_empty = {:?}", ciphertext_empty);
        let decrypted_msg_empty = aes256.decrypt(&ciphertext_empty, &key_empty, &key_iv).unwrap();
